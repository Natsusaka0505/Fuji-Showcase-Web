...
export function AuditPanel() {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <Card
        title={t("災害資料可稽核面板")}
        subtitle={t("每港附官方即時查詢連結,可當場比對")}
        right={<Chip label={t("9 港")} tone="quantum" />}
      ...
      >
          ...
          <table className="w-full min-w-[440px] text-left">
            <thead>
              <tr className="text-[10px] uppercase tracking-wide text-ink-faint">
                ...
                <th className="pb-1 font-normal">{t("地震 M5.0+")}</th>
                <th className="pb-1 text-right font-normal">{t("颱風天/年")}</th>
                <th className="pb-1 pl-4 text-right font-normal">{t("查證")}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <Prose>
          {t("查詢視窗固定 2015-01-01 → 2026-07-04、半徑 300 km、規模 M5.0+。點高雄那條連結,USGS 官方回傳")}{" "}
          <Mono>243</Mono>
          {t(",與模型內建數字完全一致 —— 這就是「可稽核」的意義。")}
        </Prose>
      </Card>

      <Card title={t("四大官方資料來源")} subtitle={t("每筆已逐一驗證,可寫進論文")}>
        <ul>
          {audit.sources.map((s) => (
      ...
      <Card title={t("模型出處")} subtitle={t("網站內每個數字怎麼來的")}>
        <dl>
          {([
            [t("問題實例"), meta.problem_name],
            [t("Pauli 項"), t("{n}({l} linear + {q} quadratic)", { n: meta.num_pauli_terms, l: meta.num_linear_terms, q: meta.num_quadratic_terms })],
        ...
        <Prose>
          {t("此式由 {n} 個 Pauli 項逆向還原,與平台", { n: meta.num_pauli_terms })}{" "}
          <Mono>cost_of_z</Mono>{" "}
          {t("的最大誤差為")}{" "}
          <Mono>{meta.model_verified_max_abs_err.toExponential(1)}</Mono>
          {t("(浮點極限)。網站因此能在瀏覽器端即時重算,而不是查表播放。")}
        </Prose>
        <Caveat>{t(meta.caveat)}</Caveat>
      </Card>

      <Card
        title={t("災害參數原始檔(port_hazards_v2)")}
        subtitle={t("發表 CVaR 表的實際輸入,來自平台證據包")}
        right={<Chip label={t("原始資料")} tone="good" filled />}
        className="xl:col-span-2"
      >
        <div className="overflow-x-auto">
          <table className="w-full min-w-[520px] text-left">
            <thead>
              <tr className="text-[10px] uppercase tracking-wide text-ink-faint">
                <th className="pb-1 font-normal">{t("港")}</th>
                <th className="pb-1 text-right font-normal">{t("地震 λ/年")}</th>
                <th className="pb-1 text-right font-normal">{t("事件延誤(天)")}</th>
                <th className="pb-1 text-right font-normal">{t("颱風關港(天/年)")}</th>
                <th className="pb-1 text-right font-normal">{t("衝突係數")}</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={name} className="border-t border-border">
                    ...
                    <td className={`py-1.5 text-right font-mono text-xs tabular-nums ${(h.conflict_mult ?? 1) > 1 ? "font-bold text-warn" : "text-ink-dim"}`}>
                      ...
                      {h.conflict_expected_delay_days != null && (
                        <span className="ml-1 text-[10px] text-warn">{t("+{n} 天", { n: h.conflict_expected_delay_days })}</span>
                      )}
                    </td>
                  </tr>
                );
        ...
        <Prose>
          {t("擬合模型與原始檔的對照:蘇伊士延誤擬合")}{" "}
          <Mono>{cvar.mc_model.suez_delay_days.toFixed(4)}</Mono>{" "}
          {t("天,原始檔寫")} <Mono>7.5</Mono>
          {t(";地震事件延誤擬合")}{" "}
          <Mono>{cvar.mc_model.quake_impact_days.toFixed(4)}</Mono>{" "}
          {t("天,原始檔寫")} <Mono>0.792</Mono>
          {t("。逆向還原的結構被原始資料直接證實 —— 這兩個數字先前只能靠「筆記中獨立寫下的 7.5 天」旁證。")}
        </Prose>
        <Caveat>
          {t("方法(JMA):200 km 內風暴點,10 分鐘最大風速 <64kt 記 1 天、64–95kt 記 3 天、≥96kt 記 7 天,統計 2015–2025。引用:USGS ANSS ComCat、RSMC Tokyo Best Track(JMA)、Suez baseline。原始生成器為 montecarlo_cvar.py(K=10,000、30 天窗、$267k/天、seed 7);本站 risk.ts 為對發表數字的行為等效擬合,分布形狀不同(Gamma vs Exponential),平均誤差 0.15%。")}
        </Caveat>
      </Card>

      <Card title={t("score 拆解:已由原始碼驗證")} subtitle={t("取代先前的 cost_norm 假設")} className="xl:col-span-2">
        <Prose>
          {t("平台證據包內的")} <Mono>build_ising_40q.py</Mono>{" "}
          {t("揭露了邊分數的生成公式:")}
          <Mono>score = haversine/20000 + λ·port_risk_dest/max + |N(0,0.03)|</Mono>
          {t("(λ = 0.4)。16 條邊逐一對帳:距離與風險項精確重算,市場擾動殘差全部落在半正態界內(16/16)。側欄的「風險權重 λ」滑桿即基於此拆解 —— λ = 0.40 時與比賽實例逐位一致。舊的")}{" "}
          <Mono>cost_norm_hypothesis</Mono>
          {t("(相關係數 0.52 的反推假設)已退役。")}
        </Prose>
        <Caveat>
          {t("仍未驗證:α/β/γ₁γ₂γ₃ 五分項(cost/time/geo/port/weather)在 16q 實例的逐邊數值不存在 —— 該實例的拆解是 dist+λ·risk+market;五分項屬 0.9281 家族,見「聯運」分頁。")}
        </Caveat>
      </Card>
    </div>
  );
