...
function GroverCard({ model, energies, optimum }: { model: Q9Model; energies: Float64Array; optimum: number }) {
  ...
  return (
    <Card
      title="Grover 自適應搜尋"
      subtitle="BBHT 外圈｜每次調整都在你的瀏覽器重跑"
      right={<Chip label={hit ? "命中最優" : "未命中"} tone={hit ? "good" : "warn"} filled={hit} />}
    >
      <LineChart
      ...
      <div className="mt-2 flex gap-3">
        <Stat label="最佳" value={result.bestEnergy.toFixed(4)} tone={hit ? "good" : "warn"} />
        <Stat label="總旋轉" value={String(result.totalRotations)} tone="quantum" />
        <Stat label="改善次數" value={String(result.trail.filter((s) => s.improved).length)} />
      </div>
      {best.complete && <Route iso={best.routeIso} className="mt-3" />}

      <div className="mt-4 grid gap-x-6 sm:grid-cols-2">
        <Slider label="budget（BBHT 輪次）" value={budget} min={1} max={120} step={1}
          onChange={setBudget} format={(v) => v.toFixed(0)}
          hint="平台 40q 那次只有 1 輪" />
        <Slider label="shots（每輪取樣）" value={shots} min={8} max={256} step={8}
          onChange={setShots} format={(v) => v.toFixed(0)} tone="quantum" />
        <Slider label="亂數種子" value={seed} min={1} max={40} step={1}
          onChange={setSeed} format={(v) => v.toFixed(0)} tone="faint"
          hint="GAS 是機率演算法,換種子結果會變" />
        <div className="mb-4">
      ...
      <Prose>
        藍線是門檻下降的時刻。每次採到更好的解,門檻就收緊、被標記的態變少,下一輪需要更多次旋轉才找得到 ——
        這就是 BBHT 逐步加大 r 的機制。把 <Mono>budget</Mono> 拉到 20 以下會看到它來不及收斂,
        正是平台 40q 那次 <Mono>budget=1</Mono> 只拿到 {result40.best_objective.toFixed(2)} 的原因。
      </Prose>
      <Caveat>
        這裡的 oracle 直接作用在 16 個 key qubit 上(用古典已知的能量判斷)。硬體上同一個 oracle 要靠
        QFT 算術把 w(x) 寫進 value register 再讀符號位,那才是 16 key + 24 val = 40 qubit 的來源。
        兩者標記的態完全相同,放大動力學一致;差別在閘數 —— 那正是 40q 單輪要 {fmtHours(result40.wallclock_sec)} 的原因。
      </Caveat>
    </Card>
  );
}
...
function AmplificationCard({ energies }: { energies: Float64Array }) {
  ...
  return (
    <Card
      title="振幅放大解剖"
      subtitle="固定門檻下,每旋轉一次測一次命中機率"
      right={<Chip label={`${nMarked} 個標記態`} tone="quantum" />}
    >
      <LineChart
      ...
      />
      <div className="mt-2 flex gap-3">
        <Stat label="起始機率" value={`${(probs[0] * 100).toFixed(3)}%`} tone="faint" />
        <Stat label="峰值" value={`${(peak * 100).toFixed(1)}%`} tone="gold" />
        <Stat label="峰值位置" value={String(peakAt)} unit="轉" tone="quantum" />
        <Stat label="理論 √(N/M)" value={String(optimalR)} unit="轉" />
      </div>

      <div className="mt-4 grid gap-x-6 sm:grid-cols-2">
        <Slider label="門檻 y" value={threshold} min={-97.5} max={-40} step={0.5}
          onChange={setThreshold} format={(v) => v.toFixed(1)}
          hint="越接近最優,被標記的態越少、越難找" tone="bad" />
        <Slider label="旋轉上限" value={rotations} min={5} max={300} step={5}
          onChange={setRotations} format={(v) => v.toFixed(0)} tone="quantum" />
      </div>

      <Prose>
        實測峰值落在 <Mono>{peakAt}</Mono> 轉,理論估計 <Mono>{optimalR}</Mono> 轉 ——
        這是真的在跑 2<sup>16</sup> 個振幅,不是把公式畫出來。繼續轉下去機率會掉回來(過度旋轉),
        這也是 Grover 必須知道何時停手的原因。把門檻拉到 -97.4 會只剩 1 個標記態,需要約 200 轉。
      </Prose>
    </Card>
  );
}
...
function QaoaCard({ energies, optimum }: { energies: Float64Array; optimum: number }) {
  ...
  return (
    <Card
      title="QAOA"
      subtitle={`p=${p} 深度｜完整 2^16 態向量演化`}
      right={<Chip label={pending ? "計算中…" : `${result.evaluations} 次評估`} tone="quantum" />}
    >
      <LineChart
        series={[{ values: result.convergence, color: "var(--color-quantum)" }]}
        refLines={[{ value: optimum, color: "var(--color-good)", label: "最優" }]}
        xLabel="Nelder-Mead 迭代"
        yLabel="⟨C⟩"
        height={140}
      />
      <div className="mt-2 flex gap-3">
        <Stat label="期望值" value={result.bestExpectation.toFixed(2)} tone="quantum" />
        <Stat label="採樣最佳" value={result.bestSampledEnergy.toFixed(4)}
          tone={sampledHit ? "good" : "warn"} />
        <Stat label="P(最優)" value={`${(result.optimumProbability * 100).toFixed(3)}%`} tone="gold" />
      </div>

      <div className="mt-4 grid gap-x-6 sm:grid-cols-2">
        <Slider label="p（電路深度）" value={p} min={1} max={5} step={1}
          onChange={setP} format={(v) => v.toFixed(0)}
          hint="每層 = 一個成本相位 + 一個混合層" tone="quantum" />
        <Slider label="最大迭代" value={maxIter} min={10} max={150} step={5}
          onChange={setMaxIter} format={(v) => v.toFixed(0)} />
        <Slider label="亂數種子" value={seed} min={1} max={40} step={1}
          onChange={setSeed} format={(v) => v.toFixed(0)} tone="faint"
          hint="初始角度隨機,會落進不同局部解" />
        <div className="mb-4">
      ...
      <div className="mt-2">
        <div className="mb-2 text-[10px] uppercase tracking-wide text-ink-faint">最可能的量測結果</div>
      ...
      <Prose>
        ⟨C⟩ 是能量期望值,不是採樣到的最低值 —— 均勻疊加態的期望值是 0,優化後降到約 -60,
        但採樣仍會抽到 <Mono>{result.bestSampledEnergy.toFixed(4)}</Mono>。這就是報告裡
        「收斂到 -44.79、但命中 -97.4936」看似矛盾的原因:兩個是不同的量。
      </Prose>
    </Card>
  );
}
...
function SaCard({ energies, optimum }: { energies: Float64Array; optimum: number }) {
  ...
  return (
    <Card
      title="模擬退火（古典對照）"
      subtitle="單位元翻轉 + 幾何降溫"
      right={<Chip label={hit ? "命中最優" : `近似比 ${ratio.toFixed(4)}`} tone={hit ? "good" : "warn"} />}
    >
      <LineChart
      ...
      <div className="mt-2 flex gap-3">
        <Stat label="最佳" value={result.bestEnergy.toFixed(4)} tone={hit ? "good" : "warn"} />
        <Stat label="近似比" value={ratio.toFixed(4)} tone="gold" />
        <Stat label="接受率" value={`${((result.accepted / iterations) * 100).toFixed(0)}%`} />
      </div>

      <div className="mt-4 grid gap-x-6 sm:grid-cols-2">
        <Slider label="迭代數" value={iterations} min={200} max={20000} step={200}
          onChange={setIterations} format={(v) => v.toLocaleString()} tone="warn" />
        <Slider label="起始溫度" value={startTemp} min={1} max={200} step={1}
          onChange={setStartTemp} format={(v) => v.toFixed(0)} tone="warn"
          hint="太低會卡在局部解,太高等於亂數搜尋" />
        <Slider label="亂數種子" value={seed} min={1} max={40} step={1}
          onChange={setSeed} format={(v) => v.toFixed(0)} tone="faint" />
      </div>

      <Prose>
        古典基準線。報告中 SA 拿到 -96.8297(近似比 0.9932),QAOA 則命中 -97.4936。
        把迭代數拉低或起始溫度調小,可以看到它卡在局部解 —— 那正是組合最佳化的難處。
      </Prose>
    </Card>
  );
}
