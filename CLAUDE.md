...
### 待接:app_data.json(v1,8 月主成果)

40q 命中全域最佳、四情境切換(base/A 直達停航/B +高雄封港/C +紅海中斷)、422 條航線全排行 + q_count。**檔案在團隊聊天室,尚未落地本機**(311 KB,SHA-256 `fb70cff6…3fefd6db`)。格式與驗收清單見 Notion「8/29 App 交接包」(base Top-1 = SIN→LA 0.8794 q×884;A Top-1 = SIN→Colombo→LA 1.2534;B 135/422;C 33/422)—— 驗收清單直接寫成 verify 斷言。平台 job:7953691/7954111/7954358/7954112。
...
7. **全站常駐 footer 聲明**(比賽誠實紅線):「富士通 1024×FX700 無雜訊古典態向量模擬,非量子實機,不宣稱量子優勢」,三語,不可移除。
8. **8 月新資料(app_data v1/v2)不上美元數字。** 帶 USD 的風險 CSV 未過查證;新資料只顯示標準化模型成本/風險分數。既有風險分頁的 USD 屬 16q 已驗證體系(對帳過發表數字),不受此限。
9. **v1 交接包紅線**(接入 app_data.json 時):跨情境成本不可比(offset 不同),介面不得放跨情境成本比較;「即時」= 查快取,不是量子毫秒算完。
